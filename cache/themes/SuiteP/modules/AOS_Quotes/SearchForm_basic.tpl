...
<input type='text' name='{$fields.name_basic.name}' 
	...
	<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 search_fields_basic">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
						<label for='billing_account_basic' >{sugar_translate label='LBL_BILLING_ACCOUNT' module='AOS_Quotes'}</label>
					</div>
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-8">
		
<input type="text" name="{$fields.billing_account_basic.name}"  class="sqsEnabled"   id="{$fields.billing_account_basic.name}" size="" value="{$fields.billing_account_basic.value}" title='' autocomplete="off"  >
<input type="hidden"  id="{$fields.billing_account_id_basic.name}" value="{$fields.billing_account_id_basic.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.billing_account_basic.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.billing_account_basic.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"billing_account_id_basic","name":"billing_account_basic"}}{/literal}, "single", true);'><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.billing_account_basic.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.billing_account_basic.name}.value = ''; this.form.{$fields.billing_account_id_basic.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
